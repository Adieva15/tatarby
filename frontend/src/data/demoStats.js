...
export function getNextMilestone(stats) {
  const milestones = [
    ...
    {
      name: "500 слов",
      current: stats.total_words_learned,
      target: 500,
      type: "words",
    },
    {
  ...
  return (
    milestones.find((m) => m.current < m.target) ||
...
export const demoReadingText = {
  ...
  translation:
    "Ананар вернулся домой и помог маме. Он встал рано утром, вымыл пол, заварил чай. Затем он читал книгу и учил новые слова. Вечером он играл с друзьями на улице.",
};
...
export const demoEssayTask = {
  title: "Сочинение дня",
  description:
    "Напиши 3–5 предложений о своём дне, используя выученные сегодня слова.",
...
export function checkEssay(text) {
  ...
  const missingWords = demoEssayTask.requiredWords.filter(
    (w) => !lowerText.includes(w.toLowerCase()),
  );
  ...
  if (usedWords.length === demoEssayTask.requiredWords.length) score += 50;
    ...
    score += Math.round(
      (usedWords.length / demoEssayTask.requiredWords.length) * 50,
    );
  ...
  return {
    ...
    feedback: getFeedback(
      score,
      usedWords.length,
      demoEssayTask.requiredWords.length,
    ),
  };
}
...
function getFeedback(score, used, total) {
  if (score >= 90)
    return "Отличная работа! Все слова использованы, объём хороший.";
  if (score >= 70)
    return "Хорошо! Но можно использовать больше выученных слов.";
  if (score >= 50)
    return `Неплохо. Использовано ${used} из ${total} обязательных слов.`;
...
export const adaptiveTexts = {
  A1: {
    ...
    sentences: [
      ...
      {
        tat: "Кич белән ул дуслары белән уйнады.",
        rus: "Вечером он играл с друзьями.",
      },
    ],
  },
  A2: {
    ...
    sentences: [
      {
        tat: "Ананар иртә белән өйгә кайтты һәм әнисенә булышты.",
        rus: "Ананар вернулся домой рано утром и помог маме.",
      },
      {
        tat: "Ул идән юды, чәй ясады һәм өстәл әзерләде.",
        rus: "Он вымыл пол, заварил чай и накрыл на стол.",
      },
      {
        tat: "Аннары ул китап укыды һәм яңа сүзләр өйрәнде.",
        rus: "Затем он читал книгу и учил новые слова.",
      },
      {
        tat: "Кич белән ул дуслары белән урамда уйнады.",
        rus: "Вечером он играл с друзьями на улице.",
      },
    ],
  },
  B1: {
    ...
    sentences: [
      {
        tat: "Ананар иртә белән торып, өй эшләренә әнисенә булышты: идән юды, чәй ясады, өстәл әзерләде.",
        rus: "Встав рано утром, Ананар помог маме по дому: вымыл пол, заварил чай, накрыл на стол.",
      },
      {
        tat: "Аннары ул китап укып, яңа сүзләр өйрәнде һәм аларны дәфтәренә язып куйды.",
        rus: "Затем он читал книгу, учил новые слова и записывал их в тетрадь.",
      },
      {
        tat: "Кич белән ул дуслары белән урамда озак уйнады, аннары өйгә кайтып йоклады.",
        rus: "Вечером он долго играл с друзьями на улице, затем вернулся домой и уснул.",
      },
    ],
  },
};
