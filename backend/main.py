...
import uuid
from database.models.Essay import EssayModel
from utils.translate import translate, analyze_morphology
...
from database.models.WordResponse import WordResponse, WordRequest
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://127.0.0.1:5173",
        "http://localhost:3000",     # если Next.js
    ],
    allow_credentials=True,          # ← обязательно, если шлёшь куки/refresh
...
@app.post("/api/word")
def search_word(req: WordRequest, user_id: int = Depends(get_current_user), response_model=WordResponse):
    word = req.word.strip()
    if not word:
    ...
    return {"word": translation, "translation":translate, "morph": morph}


@app.post("/api/essay")
def essay(text_tat: EssayModel, user_id: int = Depends(get_current_user), db: Session = Depends(getdb)):
    user = db.query(User).filter(User.id == user_id).first()
    text_ru = translate(text_tat.text_tat)
    ans = client.check_essay(text_ru, text_tat.text_tat, user.language_level)
    result_text = json.loads(ans["choices"][0]["message"]["content"])
    return result_text
